...
public class ClaseProvisionalConMetodoDeCerebro {
	
	
	private int pos;
	
	
	public ClaseProvisionalConMetodoDeCerebro (int pPos) {this.pos = pPos;}
	

	
	// pos 0 es la de abajo, pos 1 es la de arriba en el tablero
	
	// Necesita mas testeo por las nuevas normas introducidas, ha pasado todos los casos de prueba por ahora
	...
	public boolean esFichaAliada(Ficha pFicha) {
		...
		if (this.pos == 0) {
			...
			if(pFicha instanceof PeonDeJugadorDos || pFicha instanceof DamaDeJugadorDos) {
				return true;
			} else {return false;}
			}
	...
	protected int getPos() {return this.pos;}
	
		
	
	public void cambioDeCerebro() {
		
		
		if (this.pos == 0) {this.pos = 1;} else {this.pos = 0;}
		
		
		System.out.append("Cambio de turno, le toca a");
		
		if (this.pos == 0) {System.out.append(" fichas a/A");
		}else {System.out.append(" fichas b/B");}
	...
	public void pierde () {
		...
		if (this.pos == 0) {System.out.println ("Pierde A/a  |  Gana B/b");}
		else {System.out.println ("Gana A/a |  Pierde B/b");}
	}
	...
	public boolean esVacio (Ficha pFicha) {
		
		if (pFicha instanceof Vacio) {return true;} else {return false;}
	}
	...
	public Movimiento cerebroUno() {
		...
		while (f != 8 && !decidido ) {
			...
			while (c != 8 && !decidido) {
				...
				if (this.esPeonAliado(Tablero.getMiTablero().obtenerFicha(f,c)) && !decidido && this.pos == 0) { 
			    ...
			    // Si encuentra peon de J2 siendo J2
					
        	} else if (this.esPeonAliado(Tablero.getMiTablero().obtenerFicha(f,c)) && !decidido && this.pos == 1) {

					if (f+2 < 8 && c+2 < 8 && !decidido) {
			...
			while (c < 8 && !decidido) {
				...
				if (this.esPeonAliado(Tablero.getMiTablero().obtenerFicha(f,c))) {
					...
					if (this.comible(f,c, 0, false, false)) {	
						...
						} else if (this.pos == 1 && f != 7) { // Siendo J2
		...
		while (f < 8 && !decidido && !decidioAlgunaVez) {
			...
			if(this.esPeonAliado(Tablero.getMiTablero().obtenerFicha(f, c))) {
				...
				} else if (this.pos == 1 && f != 7) { // Si es J2
	...
	public int queDiagonalMovio(Movimiento pMovimiento) {
		
		/* Pre: un movimiento
		   Post: Dice en que diagonal se movio la ficha (1 arr izq, 2 arr dcha,
		       3 abo izq, 4 abo dcha, 0 no hay diagonal)
		
		   El movimiento se hace empezando en la posición del final y desplazandose en
		   cada diagonal hasta la posición del principio, si se encuentra, se devuelve
		   el integer correspondiente y el método acaba, si no se encuentra en dicha diagonal
		   se pasa a la siguiene  */
		...
		f = pMovimiento.getFilaAcabado() - 1;
		c = pMovimiento.getColumnaAcabado() - 1;
		
		while ((pMovimiento.getFilaEmpezado() != f || pMovimiento.getColumnaEmpezado() != c) && f > -1 && c > -1 && diagonal == 0) {
			if (pMovimiento.getFilaEmpezado() == f && pMovimiento.getColumnaEmpezado() == c) {diagonal = 4;}
			f--;
			c--;
		...
		while ((pMovimiento.getFilaEmpezado() != f || pMovimiento.getColumnaEmpezado() != c) && f > -1 && c < 8 && diagonal == 0) {
			if (pMovimiento.getFilaEmpezado() == f && pMovimiento.getColumnaEmpezado() == c) {diagonal = 3;}
			f--;
			c++;
		...
		while ((pMovimiento.getFilaEmpezado() != f || pMovimiento.getColumnaEmpezado() != c) && f < 8 && c > -1 && diagonal == 0) {
			if (pMovimiento.getFilaEmpezado() == f && pMovimiento.getColumnaEmpezado() == c) {diagonal = 2;}
			f++;
			c--;
		...
		while ((pMovimiento.getFilaEmpezado() != f || pMovimiento.getColumnaEmpezado() != c) && f < 8 && c < 8 && diagonal == 0) {
			if (pMovimiento.getFilaEmpezado() == f && pMovimiento.getColumnaEmpezado() == c) {diagonal = 1;}
			f++;
			c++;
	...
	public  boolean comible (int pFila, int pColumna, int pDireccion, boolean pDamaInexistente, boolean pComioAdyecente) {
		...
		if (pFila != 0 && pFila != 7 && pColumna != 0 && pColumna != 7) { // No puede ser comido en las esquinas del tablero
			...
			if (this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila+1, pColumna+1)) || pDireccion == 4) { 
				...
				if (this.pos == 0 && this.esPeonEnemigo(Tablero.getMiTablero().obtenerFicha(pFila-1, pColumna-1)) && pDireccion != 1) {
					comible = true;
	...
	//		if (pDamaInexistente) {pasadoPorDama = false;} else {pasadoPorDama = true;} <-- Codigo no necesario
			...
			if ((this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila+1, pColumna-1)) || pDireccion == 3) && ! comible) {
				...
				if (this.pos == 0 && this.esPeonEnemigo(Tablero.getMiTablero().obtenerFicha(pFila-1, pColumna+1 )) && pDireccion != 2) {
					comible = true;
				}
		...
		//	if (pDamaInexistente) {pasadoPorDama = false;} else {pasadoPorDama = true;} <-- Código no necesario
			...
			if ((this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila-1, pColumna+1)) || pDireccion == 2) && !comible) {
				...
				if (this.pos == 1 && this.esPeonEnemigo(Tablero.getMiTablero().obtenerFicha(pFila+1, pColumna-1)) && pDireccion != 3) {
					comible = true;
				}
	...
	//		if (pDamaInexistente) {pasadoPorDama = false;} else {pasadoPorDama = true;} <-- Código no necesario
			...
			if ((this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila-1, pColumna-1)) || pDireccion == 1) && !comible) {
				...
				if (this.pos == 1 && this.esPeonEnemigo(Tablero.getMiTablero().obtenerFicha(pFila+1, pColumna+1)) && pDireccion !=4) {
					comible = true;
	...
	public  boolean encadenar(int pFila, int pColumna, int pDireccion, boolean pEsDama) {
		...
		if (pFila > 1 && pColumna > 1 && pDireccion != 1 && (pEsDama || this.pos == 0)) {
			if (this.esFichaEnemiga(Tablero.getMiTablero().obtenerFicha(pFila-1, pColumna-1)) && this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila-2, pColumna-2))) {
		...
		if (pFila > 1 && pColumna < 6 && pDireccion != 2 && !encadenar && (pEsDama || this.pos == 0)) {
			if (this.esFichaEnemiga(Tablero.getMiTablero().obtenerFicha(pFila-1, pColumna+1)) && this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila-2, pColumna+2))) {
		...
		if (pFila < 6 && pColumna > 1 && pDireccion != 3 && !encadenar && (pEsDama || this.pos == 1)) {
			if (this.esFichaEnemiga(Tablero.getMiTablero().obtenerFicha(pFila+1, pColumna-1)) && this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila+2, pColumna-2))) {
		...
		if (pFila < 6 && pColumna < 6 && pDireccion != 4 && !encadenar && (pEsDama || this.pos == 1)) {
			if (this.esFichaEnemiga(Tablero.getMiTablero().obtenerFicha(pFila+1, pColumna+1)) && this.esVacio(Tablero.getMiTablero().obtenerFicha(pFila+2, pColumna+2))) {
